...
ROOT=Path(__file__).resolve().parents[1];STATE=ROOT/'artifacts/p3-73-integration.json'
...
def main():
 ...
 wait_state('artifacts/p3-70-continuation.json')
 STATE.write_text(json.dumps({'status':'RUNNING','source':SOURCE})+'\n')
 mailbox='artifacts/p3-73-mailbox'
 def planner():run(1,'pipelined_planner_probe',['--role','planner','--source',SOURCE,'--mailbox',mailbox,'--out','artifacts/p3-73-planner'])
 def execute():run(3,'pipelined_planner_probe',['--role','executor','--source',SOURCE,'--mailbox',mailbox,'--out','artifacts/p3-73-executor'])
 with ThreadPoolExecutor(max_workers=2) as pool:
  a=pool.submit(planner);b=pool.submit(execute);b.result();a.result()
 with ThreadPoolExecutor(max_workers=2) as pool:
  a=pool.submit(run,1,'pipelined_planner_probe',['--role','executor','--fixed-initial-plan','artifacts/p3-73-mailbox/phase-00.json','--source',SOURCE,'--mailbox','artifacts/p3-73-control-mailbox','--out','artifacts/p3-73-control'])
  b=pool.submit(run,3,'render_online_probe',['--source','artifacts/p3-73-executor','--out','artifacts/p3-73-online-playback']);a.result();b.result()
 run(3,'render_online_probe',['--source','artifacts/p3-73-control','--out','artifacts/p3-73-control-playback'])
 reports={}
