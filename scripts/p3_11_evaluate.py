...
def worker(item):
    ...
    out=f'artifacts/p3-11-mapped-seed{seed}-hops{hops}'
    cmd=[sys.executable,'scripts/run_job.py','--gpu',str(gpu),'--timeout','300','evaluate',
         ...
         '--map-goal-forward-m',str(.15*hops),'--mapped-contact-progress','--map-course-boundary','--episodes','64','--diagnostics','--video',
         '--video-envs','64','--video-camera-side','4','--research-tag','phase:P3',
         '--research-tag','step:p3-11-extended-course','--research-tag','condition:'+condition]
    code=subprocess.run(cmd,cwd=ROOT).returncode
    if code:return dict(condition=condition,seed=seed,hops=hops,exit_code=code)
    checked=check(ROOT/out)
    r=json.loads((ROOT/out/'evaluation.json').read_text())
    return dict(condition=condition,seed=seed,hops=hops,exit_code=0,successes=r['successes'],
...
if __name__=='__main__':
    ...
    plan=[(1,6,0),(2,6,1),(1,8,2),(2,8,3)]
    for s,h,g in plan:
        p=ROOT/f'artifacts/p3-11-mapped-seed{s}-hops{h}'
    ...
    (ROOT/'docs/p3-11-results.json').write_text(json.dumps(r,indent=2)+'\n')
