"""P2-28 paired results and conservative first-touch geometry diagnostics."""
import json
import argparse
...
def main():
    ...
    for seed in range(4):
        policy_hash = None
        for mode in modes:
            path = ROOT/'artifacts'/f'p2-28-material-{mode}-seed{seed}'
    ...
    (ROOT/f'docs/p2-28-summary.json').write_text(json.dumps(summary, indent=2)+'\n')
    lines = ['# P2-28 고정 정책의 지지면 전이', '',
    ...
    (ROOT/f'docs/p2-28-results.md').write_text('\n'.join(lines))
    print('\n'.join(lines))
