...
CASES=[(experiment,rng) for experiment in ('p2-20','p2-22') for rng in (20000,20001)]

def worker(seed):
    for experiment,rng in CASES:
        mode='sampled'
        train=f'artifacts/{experiment}-deck-seed{seed}'
        out=f'artifacts/p2-23-{experiment}-seed{seed}-sampled-rng{rng}'
        command=[sys.executable,'scripts/run_job.py','--gpu',str(seed),'--timeout','240','evaluate',
            ...
            '--research-tag','step:p2-23-bounded-action-sampling','--research-tag','action:'+mode]
...
if __name__=='__main__':
    for seed in range(4):
        for experiment,rng in CASES:
            p=ROOT/'artifacts'/f'p2-23-{experiment}-seed{seed}-sampled-rng{rng}'
