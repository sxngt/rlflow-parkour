"""P2-30 parent paired baseline, continuous regression and retained split jump."""
...
def commands(seed):
    cases=[(f'p2-29-split-seed{seed}',f'p2-30-parent-seed{seed}-split-mixed','split',[0.,.15],'parent-baseline')]
    for mode in ('fixed','mixed'):
        train=f'p2-30-{mode}-seed{seed}'
    ...
    for train,out,terrain,distances,purpose in cases:
        yield [sys.executable,'scripts/run_job.py','--gpu',str(seed),'--timeout','240','evaluate',
            ...
            '--research-tag','phase:P2','--research-tag','step:p2-30-capability-retention',
...
if __name__=='__main__':
    ...
    for seed in args.seeds:
        ...
        for mode in ('fixed','mixed'):
            path=ROOT/'artifacts'/f'p2-30-{mode}-seed{seed}'
