...
ROOT=Path(__file__).resolve().parents[1];STATE=ROOT/'artifacts/p3-71-integration.json'
SOURCE='artifacts/p3-63-bridge__final-evaluation'
def wait_state(path):
...
def main():
 ...
 wait_state('artifacts/p3-68-stage3.json')
 STATE.write_text(json.dumps({'status':'RUNNING','source':SOURCE})+'\n')
 mailbox='artifacts/p3-71-mailbox'
 def planner():run(0,'pipelined_planner_probe',['--role','planner','--source',SOURCE,'--mailbox',mailbox,'--out','artifacts/p3-71-planner'])
 def execute():run(2,'pipelined_planner_probe',['--role','executor','--source',SOURCE,'--mailbox',mailbox,'--out','artifacts/p3-71-executor'])
 with ThreadPoolExecutor(max_workers=2) as pool:
  a=pool.submit(planner);b=pool.submit(execute);b.result();a.result()
 with ThreadPoolExecutor(max_workers=2) as pool:
  a=pool.submit(run,0,'online_planner_probe',['--role','executor','--fixed-plan','--threads','1','--source',SOURCE,'--mailbox','artifacts/p3-71-control-mailbox','--out','artifacts/p3-71-control'])
  b=pool.submit(run,2,'render_online_probe',['--source','artifacts/p3-71-executor','--out','artifacts/p3-71-online-playback']);a.result();b.result()
 run(2,'render_online_probe',['--source','artifacts/p3-71-control','--out','artifacts/p3-71-control-playback'])
 reports={}
 for arm,source in [('online','p3-71-executor'),('control','p3-71-control')]:
  report=json.loads((ROOT/'artifacts'/source/'execution.json').read_text());reports[arm]=report
  rendered=ROOT/'artifacts'/('p3-71-'+arm+'-playback');runmeta=json.loads((rendered/'run.json').read_text())
  out=ROOT/'result'/('P3-71_30.75cm갭_24구간_'+arm+'_네착지물리계획_상태재생');out.mkdir(exist_ok=False)
