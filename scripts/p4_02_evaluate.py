...
def evaluation_plan():
    ...
    for seed in (1,2):
        for condition in ('lowfriction','control'):
            source=f'artifacts/p4-02-{condition}-seed{seed}'
            for suite in ('uniform8','friction0p2','friction0p05','continuous'):
                out=f'artifacts/p4-02-eval-{condition}-seed{seed}-{suite}'
                command=[sys.executable,'scripts/run_job.py','--gpu',str(len(result)%4),'--timeout','300','evaluate',
                    ...
                    '--research-tag','phase:P4','--research-tag','step:p4-02-friction-training',
...
if __name__=='__main__':
    ...
    for condition in ('lowfriction','control'):
        for seed in (1,2):gate(f'p4-02-{condition}-seed{seed}',1,800,1024)
    for item in plan:
        p=ROOT/item['out']
        if p.exists() or p.with_suffix('.log').exists():raise RuntimeError('Existing attempt '+str(p))
    (ROOT/'artifacts/p4-02-evaluation-plan.json').write_text(json.dumps(plan,indent=2)+'\n')
