...
from parkour.shared_terrain import build_long_shared_course,world_point
...
def main():
    ...
    out=ROOT/'result/scenarios/ten-transfer-difficulty-designs';out.mkdir(parents=True,exist_ok=True)
    fig=plt.figure(figsize=(15,5));entries=[]
    for index,level in enumerate(('easy','medium','hard')):
        layout=build_long_shared_course(level,1)
        ...
        for i,s in enumerate(layout['surfaces']):
            ...
            ax.add_collection3d(Poly3DCollection(faces,facecolor=plt.cm.viridis(i/10),edgecolor='#334155',linewidth=.5,alpha=.9))
            cx,cy,cz=s['top_center_m'];ax.text(cx,cy,cz+.08,str(i),fontsize=8)
        ...
        ax.set_title(f'{level.upper()} | 10 transfers\npath {layout["nominal_path_length_m"]:.2f} m | max height change {max_step:.2f} m')
        entries.append({'level':level,'geometry_seed':1,'transfers':10,'nominal_path_length_m':layout['nominal_path_length_m'],'max_center_height_change_m':max_step,'geometry':level+'.json'})
