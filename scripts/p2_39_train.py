"""Four-seed coverage experiment, gated by completed runtime and archive checks."""
...
def gates():
    records = []
    for name, updates, first in [('coverage-training-smoke', 12, 1),
                                 ('coverage-training-resume', 2, 13),
                                 ('endpoint-regression-smoke', 12, 1),
                                 ('coverage-profile-seed2', 60, 1)]:
        path = ROOT / ('artifacts/p2-39-' + name)
        result = check(path)
    ...
    config = json.loads((ROOT / 'configs/p2-39-coverage.json').read_text())
    ...
    for seed in range(4):
        baseline = ROOT / f'artifacts/p2-38-continuous-seed{seed}'
        ...
        for c in (old, new):
            ...
            c.pop('seed', None)
        assert old['retention_training']['single_goal_choices_m'] == [0., .15]
        old['retention_training']['single_goal_choices_m'] = [0., .05, .1, .15]
        assert old == new
...
def worker(seed):
    source = f'artifacts/p2-39-coverage-seed{seed}'
    cmd = [sys.executable, 'scripts/run_job.py', '--gpu', str(seed), '--timeout', '2400',
           'train', '--config', 'configs/p2-39-coverage.json', '--seed', str(seed),
...
if __name__ == '__main__':
    records = gates()
    (ROOT / 'artifacts/p2-39-training-gates.json').write_text(json.dumps(records, indent=2) + '\n')
    for seed in range(4):
        path = ROOT / f'artifacts/p2-39-coverage-seed{seed}'
