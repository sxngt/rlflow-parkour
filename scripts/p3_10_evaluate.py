...
def worker(item):
    ...
    out=f'artifacts/p3-10-{condition}-seed{seed}'
    cmd=[sys.executable,'scripts/run_job.py','--gpu',str(gpu),'--timeout','300','evaluate',
         ...
         '--map-goal-forward-m','.60','--mapped-contact-progress','--map-course-boundary','--episodes','64','--diagnostics','--video',
         '--video-envs','64','--video-camera-side','4','--research-tag','phase:P3',
         '--research-tag','step:p3-10-four-hop-course','--research-tag','condition:'+condition]
...
if __name__=='__main__':
    ...
    for c,s,g in plan:
        p=ROOT/f'artifacts/p3-10-{c}-seed{s}'
    ...
    (ROOT/'docs/p3-10-results.json').write_text(json.dumps(r,indent=2)+'\n')
