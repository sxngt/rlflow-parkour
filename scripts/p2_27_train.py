...
def worker(seed):
    out = f'artifacts/p2-27-continuous-seed{seed}'
    command = [sys.executable, 'scripts/run_job.py', '--gpu', str(seed), '--timeout', '1800',
               'train', '--config', 'configs/p2-27-continuous.json', '--seed', str(seed), '--out', out]
...
if __name__ == '__main__':
    for seed in range(4):
        path = ROOT / 'artifacts' / f'p2-27-continuous-seed{seed}'
