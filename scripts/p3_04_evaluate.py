...
def evaluation_plan():
    result=[]
    for seed in (1,2):
        for condition in ('parent','course','deck'):
            source=f'artifacts/p2-40-weighted-seed{seed}' if condition=='parent' else f'artifacts/p3-04-{condition}-seed{seed}'
            for suite in ('course','deck','continuous'):
                out=f'artifacts/p3-04-eval-{condition}-seed{seed}-{suite}'
                command=[sys.executable,'scripts/run_job.py','--gpu',str(len(result)%4),'--timeout','300','evaluate',
                    ...
                    '--chain-hops','1' if suite=='continuous' else '2',
                    '--support-mode',suite,'--support-matched-material',
                    ...
                    '--research-tag','phase:P3','--research-tag','step:p3-04-course-training',
                ...
                if suite=='course':command+=['--map-goal-forward-m','.30']
...
if __name__=='__main__':
    ...
    for condition in ('course','deck'):
        for seed in (1,2):gate(f'p3-04-{condition}-seed{seed}',1,800,1024)
    for seed in (1,2):audit(ROOT/f'artifacts/p2-40-weighted-seed{seed}')
    for item in plan:
        p=ROOT/item['out']
        if p.exists() or p.with_suffix('.log').exists():raise RuntimeError('Existing attempt '+str(p))
    (ROOT/'artifacts/p3-04-evaluation-plan.json').write_text(json.dumps(plan,indent=2)+'\n')
