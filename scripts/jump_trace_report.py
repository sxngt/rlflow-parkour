"""Compare the same first development command across explicitly listed jump runs."""
...
def main():
 ...
 fig,axes=plt.subplots(2,len(conditions),figsize=(10*len(conditions),7),squeeze=False);details=[];scenario=None
 for entry in spec['runs']:
  ...
  if scenario is None:scenario=manifest['episodes'][0]
  else:assert scenario==manifest['episodes'][0],'Representative command differs'
  ...
  with np.load(p/'motion-trace.npz') as a:
   ...
   mask=valid[:,0];err=np.linalg.norm(pos[:,0,:,:2]-target,axis=-1)
   axes[0,ci].plot(t[mask],z[mask,0]*100,label=f"seed {entry['seed']}")
