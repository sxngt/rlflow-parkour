"""Complete P2-40 evaluation matrix: four new native, 16 control and 12 additional results."""
import argparse
...
def evaluation_plan():
    plan = []
    for seed in range(4):
        for condition in ('uniform', 'weighted'):
            source = (f'artifacts/p2-39-coverage-seed{seed}' if condition == 'uniform'
                      else f'artifacts/p2-40-weighted-seed{seed}')
            for suite, mode in [('chain', 'deck'), ('deck-regression', 'deck'),
                                ('regression', 'continuous'), ('split15', 'split')]:
                reused = condition == 'uniform' or suite == 'chain'
                ...
                if not reused:
                    command = [sys.executable, 'scripts/run_job.py', '--gpu', str(seed), '--timeout', '300',
                        ...
                        '--research-tag', 'phase:P2', '--research-tag', 'step:p2-40-long-goal-weight',
...
if __name__ == '__main__':
    ...
    (ROOT / 'artifacts/p2-40-evaluation-plan.json').write_text(json.dumps(plan, indent=2) + '\n')
