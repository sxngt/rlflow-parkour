"""Four bounded fresh coupled-first-touch runs, each followed by final evaluation."""
...
def worker(seed):
    out = f'artifacts/p2-18-deck-seed{seed}'
    command = [sys.executable, 'scripts/run_job.py', '--gpu', str(seed), '--timeout', '1800',
               'train', '--config', 'configs/p2-18-deck.json', '--seed', str(seed), '--out', out]
...
if __name__ == '__main__':
    for seed in range(4):
        path = ROOT / 'artifacts' / f'p2-18-deck-seed{seed}'
