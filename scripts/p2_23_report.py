...
def main():
    ...
    for experiment in ('p2-20', 'p2-22'):
        for seed in range(4):
            ...
            for mode, rng in [('mean', None), ('sampled', 20000), ('sampled', 20001)]:
                name = train.name+'__final-evaluation' if mode == 'mean' else f'p2-23-{experiment}-seed{seed}-sampled-rng{rng}'
                ...
                rows.append({'experiment': experiment, 'seed': seed, 'mode': mode, 'rng': rng,
                             'run': name, 'reused': mode == 'mean', 'successes': result['successes'],
    ...
    (ROOT/'docs/p2-23-summary.json').write_text(json.dumps(summary, indent=2)+'\n')
    lines = ['# P2-23 고정 모델 행동 샘플링 진단', '',
    ...
    (ROOT/'docs/p2-23-results.md').write_text('\n'.join(lines))
    print('\n'.join(lines))
