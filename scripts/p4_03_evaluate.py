...
def evaluation_plan():
    result=[]
    for seed in (1,2):
        for condition in ('history','zero'):
            source=f'artifacts/p4-03-{condition}-seed{seed}'
            for suite in ('uniform8','friction0p2','friction0p05','continuous'):
                out=f'artifacts/p4-03-eval-{condition}-seed{seed}-{suite}'
                command=[sys.executable,'scripts/run_job.py','--gpu',str(len(result)%4),'--timeout','300','evaluate',
                    ...
                    '--chain-hops','1' if suite=='continuous' else '8',
                    ...
                    '--research-tag','phase:P4','--research-tag','step:p4-03-history-training',
                ...
                if suite!='continuous':command+=['--map-goal-forward-m','1.2','--mapped-contact-progress','--map-course-boundary']
                if suite.startswith('friction'):command+=['--course-friction','.2' if suite=='friction0p2' else '.05','--friction-start-station','4']
...
if __name__=='__main__':
    ...
    assert len(plan)==len({p['out'] for p in plan})==16
    if args.dry_run:
        print(json.dumps(plan,indent=2));sys.exit(0)
    for condition in ('history','zero'):
        for seed in (1,2):gate(f'p4-03-{condition}-seed{seed}',1,800,1024)
    for item in plan:
        p=ROOT/item['out']
        if p.exists() or p.with_suffix('.log').exists():raise RuntimeError('Existing attempt '+str(p))
    (ROOT/'artifacts/p4-03-evaluation-plan.json').write_text(json.dumps(plan,indent=2)+'\n')
