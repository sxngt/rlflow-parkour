"""Compare frozen P2-16 checkpoints on one paired final-condition development set."""
import json
...
def main():
    rows, paired = [], None
    for seed in range(4):
        train = ROOT/'artifacts'/f'p2-16-deck-seed{seed}'
    ...
    (ROOT/'docs/p2-16-checkpoint-summary.json').write_text(json.dumps(summary, indent=2)+'\n')
    lines = ['# P2-16 중간 모델의 고정 조건 평가', '',
    ...
    (ROOT/'docs/p2-16-checkpoint-results.md').write_text('\n'.join(lines))
    print('\n'.join(lines))
