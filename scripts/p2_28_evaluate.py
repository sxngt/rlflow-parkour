"""Bounded four-seed, two-terrain evaluation from the P2-28 protocol."""
...
def commands(seed, modes=('continuous', 'split'), matched=False):
    training = f'artifacts/p2-27-continuous-seed{seed}'+('-retry1' if seed == 2 else '')
    for mode in modes:
        out = f'artifacts/p2-28-{"material-" if matched else ""}{mode}-seed{seed}'
        yield [sys.executable, 'scripts/run_job.py', '--gpu', str(seed), '--timeout', '240',
               ...
               '--research-tag', 'phase:P2', '--research-tag', 'step:p2-28-support-transfer',
               '--research-tag', 'terrain:'+mode, '--research-tag', 'purpose:fixed-policy-transfer'] + (
                   ['--support-matched-material', '--research-tag', 'step:p2-28-material'] if matched else [])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modes', nargs='+', choices=['continuous', 'split'],
                        default=['continuous', 'split'])
    ...
    if not args.matched_material:
        parser.error('P2-28 requires matched material')
    if len(set(args.modes)) != len(args.modes):
        parser.error('Duplicate modes would create duplicate attempts')
    from audit_artifacts import audit
    for seed in args.seeds:
        train=ROOT/'artifacts'/(f'p2-27-continuous-seed{seed}'+('-retry1' if seed==2 else ''))
        audit(train);audit(train.with_name(train.name+'__final-evaluation'))
