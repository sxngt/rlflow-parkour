"""Verify P3-04 evaluation comparability and summarize development results."""
import json
...
def build_report():
    training=[gate(f'p3-04-{condition}-seed{seed}',1,800,1024)
              for condition in ('course','deck') for seed in (1,2)]
    references={};rows=[]
    for item in evaluation_plan():
        ...
        if key=='course':
            ...
            assert plan['status']=='planned' and [c['forward_m'] for c in plan['contacts']]==[.15,.30]
...
if __name__=='__main__':
    r=build_report();(ROOT/'docs/p3-04-comparison.json').write_text(json.dumps(r,indent=2)+'\n')
    lines=['# P3-04 개발 평가 비교','','조건 | seed | course | deck | continuous','--- | --- | --- | --- | ---']
    for condition in ('parent','course','deck'):
        for seed in (1,2):
            scores=[next(e['successes'] for e in r['evaluations'] if e['condition']==condition and e['seed']==seed and e['suite']==suite) for suite in ('course','deck','continuous')]
    ...
    (ROOT/'docs/p3-04-comparison.md').write_text('\n'.join(lines)+'\n')
    print('\n'.join(lines))
