"""P2-26 paired results and conservative first-touch geometry diagnostics."""
import json
import argparse
...
def main():
    ...
    for seed in range(4):
        policy_hash = None
        for mode in modes:
            path = ROOT/'artifacts'/f'p2-26-material-{mode}-seed{seed}'
            ...
            assert bool(support.get('matched_material')) is True
    ...
    (ROOT/f'docs/p2-26-summary.json').write_text(json.dumps(summary, indent=2)+'\n')
    lines = ['# P2-26 고정 정책의 지지면 전이', '',
    ...
    (ROOT/f'docs/p2-26-results.md').write_text('\n'.join(lines))
    print('\n'.join(lines))
