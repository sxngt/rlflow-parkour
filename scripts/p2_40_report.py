...
from p2_40_evaluate import evaluation_plan
...
def build_report():
    budgets = []
    for seed in range(4):
        paths = [ROOT / f'artifacts/p2-39-coverage-seed{seed}', ROOT / f'artifacts/p2-40-weighted-seed{seed}']
        ...
        for cfg in configs:
            ...
            assert cfg['retention_training']['single_goal_choices_m'] == [0., .05, .1, .15]
        assert 'single_goal_weights' not in configs[0]['retention_training']
        assert configs[1]['retention_training'].pop('single_goal_weights') == [1, 1, 1, 3]
    ...
    for seed in range(4):
        for suite in ('chain', 'deck-regression', 'regression', 'split15'):
            ...
            comparisons.append(dict(seed=seed, suite=suite, episodes=64,
                uniform_training_successes=a['report']['successes'], weighted_training_successes=b['report']['successes'],
                uniform_only_successes=sum(x['success'] and not y['success'] for x, y in pairs),
                weighted_only_successes=sum(y['success'] and not x['success'] for x, y in pairs),
                uniform_training_first_hop_successes=a['first_hop_successes'],
                weighted_training_first_hop_successes=b['first_hop_successes'],
                uniform_training_by_distance=a['report'].get('by_distance'),
                weighted_training_by_distance=b['report'].get('by_distance'),
                manifests=[a['manifest'], b['manifest']]))
    total = sum(r['accounting']['new_steps'] for r in budgets)
    assert total == 157286400
    new_steps = sum(r['accounting']['new_steps'] for r in budgets if 'p2-40-' in r['run'])
    ...
    return dict(new_main_training_steps=new_steps, reused_control_steps=reused_steps,
                ...
                scope='four paired seeds, fixed final checkpoint, development evaluation; P2-39 uniform controls reused explicitly; pilots excluded')


if __name__ == '__main__':
    result = build_report()
    text = ['# P2-40 단일 목표 선택 비중 비교', '',
            'P2-39 균등 대조78,643,200step을 재사용하고 가중치 조건78,643,200step을 새로 학습했다.', '',
            '| seed | 평가군 | 균등 선택 /64 | 1:1:1:3 선택 /64 |', '|---|---|---|---|']
    for row in result['comparisons']:
        text.append(f"| {row['seed']} | {row['suite']} | {row['uniform_training_successes']} | {row['weighted_training_successes']} |")
    (ROOT / 'docs/p2-40-comparison.json').write_text(json.dumps(result, ensure_ascii=False, indent=2) + '\n')
    (ROOT / 'docs/p2-40-comparison.md').write_text('\n'.join(text) + '\n')
