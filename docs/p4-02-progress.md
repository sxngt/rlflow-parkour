...
While main training runs, README/active handoff and monitor course panel were updated and deployed on18710. Restarted web+collector so nonuniform summary loader is current; API P4 pagination and P3-12 result retrieval verified. Supplementary P3-11 closeup all64 records/trace exactly match overview.

Next prepared but NOT yet executed: scripts/p3_13_evaluate.py and docs/p3-13-protocol.md. Opt-in height-aware geometric stance planner, actual collider heights, foot target Z, launch-relative rise and landing-relative body height. Tests validate geometric height/reach/no-plan; physical first/last clone bounds and target-Z/error audits are implemented but still need simulator verification. Four probes use P3-08 fixed models: zero-height seed2 compatibility; +1cm pattern seeds1/2; -1cm pattern seed2. Run after GPU availability, and inspect zero-height equivalence before interpreting height changes. These edits preserve legacy branches for ongoing P4-02 flat training/evaluation.
