type CourseReport = {
  ...
  terminal_pose_hold?: {contract: string};
  input_action_clip_fraction?: number;
  mean_clean_airborne_count?: number;
  mean_travel_clean_airborne_count?: number; mean_travel_measured_jump_count?: number;
...
export function CourseEvaluation({report}: {report: CourseReport}) {
  if (report.required_final_index != null) return <section className="panel">
    ...
    {report.terminal_pose_hold && <p>실행 구성: 주행 RL + 종점의 고정 PD 자세 유지 · 순수 RL 종점 안정화와 별도</p>}
    ...
    {report.mean_clean_airborne_count!=null && <p>전체 episode의 네 발 비접촉 후 재접촉: 평균 {report.mean_clean_airborne_count.toFixed(2)}회 (20ms 이상, 비발 충돌 제외 · 낮은 바운딩과 하강 포함, 갭 통과 횟수와 별도)</p>}
    {report.mean_travel_clean_airborne_count!=null && <p>종점 구간 제외: 비접촉 후 재접촉 {report.mean_travel_clean_airborne_count.toFixed(2)}회 · 상승 3cm 이상 도약 {report.mean_travel_measured_jump_count?.toFixed(2)}회</p>}
