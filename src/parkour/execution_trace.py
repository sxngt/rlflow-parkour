...
class ExecutionTrace:
 def __init__(self,max_steps):self.rows=[];self.planned=[];self.max_steps=max_steps
 def capture(self,env):
  ...
  if hasattr(e,'plan'):
   ...
   p,n,i,v=planned_contacts(e)
   self.planned.append((p.detach().clone(),n.detach().clone(),i.detach().clone(),v.detach().clone()))
 ...
 def serialize(self):
  ...
  for step,values in enumerate(rows):
   ...
   for key,n in FIELDS:
    ...
    out[key]=v
   if self.planned:
    p,n,i,v=self.planned[step]
    out['planned_contacts']={'positions_w':p.cpu().tolist(),'normals_w':n.cpu().tolist(),'surface_indices':i.cpu().tolist(),'valid':v.cpu().tolist(),'horizon':4}
   result.append(out)
  return result
