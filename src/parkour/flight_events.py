...
class FlightEvents:
    def __init__(self,count,device):
        ...
        self.airborne_count=torch.zeros_like(self.count)
    def reset(self,ids):
        for value in (self.air,self.duration,self.start_z,self.peak_z,self.up,self.bad,self.had_support,self.previous_z,self.count,self.airborne_count):value[ids]=0
    def update(self,force,root_z,root_vz,nonfoot_force,dt):
        ...
        self.airborne_count+=(landed&(self.duration>=.02-1e-7)&~self.bad).long()
