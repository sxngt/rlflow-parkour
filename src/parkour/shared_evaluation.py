...
def override_course(support,level,seed,transitions=None):
    ...
    if support['layout'].get('scenario_contract')=='long_ten_gap_course_v1':
        from parkour.shared_terrain import build_ten_gap_course
        if transitions not in (None,40):raise ValueError('Ten-gap route has forty foot transfers')
        changed['layout']=build_ten_gap_course(level,seed,support['layout']['gap_scale'])
