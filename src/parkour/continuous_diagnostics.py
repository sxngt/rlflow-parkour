...
class ContinuousDiagnostics:
    def __init__(self,env,done):
        ...
        def update(dt):
            ...
            self.samples.append({'time':self.elapsed,'valid':cpu(~done),
                ...
                'root_quaternion_wxyz':cpu(e.robot.data.root_quat_w),
                'root_angular_velocity_world':cpu(e.robot.data.root_ang_vel_w),
    ...
    def close(self,out,scenario_ids):
        ...
        report={'schema_version':5,'contract':'continuous_pair_trace_v1','physics_dt_s':self.env.physics_dt,
