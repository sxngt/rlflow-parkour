...
def plan_stances(layout, nominal_xy, goal_forward_m, *, margin_m=.02,
                 min_step_m=.04, max_step_m=.16, max_hops=2):
    ...
    for dx in offsets:
        ...
        for x, y in nominal_xy:
            ...
            for s in surfaces:
                ...
                if (abs(s['top_z_m']) < 1e-8 and s['normal'] == [0.,0.,1.]
        ...
        if len(ids) != 4 or len(set(ids)) != 4:
            ...
            candidates.append({'forward_m':dx,'surface_ids':ids,
    ...
    while queue:
        ...
        for nxt,c in enumerate(candidates):
            ...
            if min_step_m-1e-7 <= delta <= max_step_m+1e-7 and nxt not in visited:
                visited.add(nxt)
                queue.append((nxt,path+[nxt]))
    return {'schema_version':1,'planner':'horizontal_translation_graph_v1',
